...
import {remote, ipcRenderer} from 'electron'; // electron has access to macos Dock
...
export default class ModalForm extends React.Component<any, any> {
  ...
  parseDuration(duration: string) {
    const mrx: RegExp = new RegExp(/([0-9][0-9]?)[ ]?m/);
    const hrx: RegExp = new RegExp(/([0-9][0-9]?)[ ]?hr/);
    let hours: number = 0;
    let mins: number = 0;
  ...
  parseIngredient(ingredients: string) {
    ...
    const units = DATA.units.join('|');
    const irx: RegExp = new RegExp(['^(\\d{1,4}(\\.\\d)?)[ ]?',
      `(${units})s? `,
      '(\\b[^\\d\\W]+(\\b[ |\\W][^\\d\\W]+\\b)*\\b)$'].join(''));
    ingredients.split(',').forEach((ingredient: string) => {
      const string = ingredient.trim();
      if (irx.test(string)) {
        const exec = irx.exec(string);
          ...
          <div className="row text-section">
            ...
            <div className="form-group">
              ...
              <textarea className="form-control"
                ...
                onChange={(e: any) => {
                  const result = e.target.value.split(',').map((s: string) => s.trim());
